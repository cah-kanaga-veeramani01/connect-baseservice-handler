...
WHERE (("validFrom" < now() AND "validTill" >= now()) OR ("validFrom" < now() AND "validTill" IS NULL)) AND "isPublished" = 1 
							  AND "legacyTIPDetailID" = tipID))
...
END;
$$ LANGUAGE 'plpgsql';
DROP TRIGGER IF EXISTS MAP_LEGACY_TIP_TO_SERVICE_USERID_TRIGGERV2 ON "attunityservice"."TIPDetailRuleOverview";
